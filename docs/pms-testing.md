...
| LocalStorageManager | SST 写入 → 读取一致性、BloomFilter 构建/查询、多路归并保留最新 Key |
| WALManager | 单盘写入 → 读取、CRC 校验正确性、Magic 检测 partial write |
| PMSerializer | SchemaId Hash 一致性、编码 → 解码 往返正确性、变长列偏移量 |
...
使用 Fake 实现替代 Paimon API：

```java
class FakePaimonSinkManager implements PaimonSinkManager {
    // 不依赖真实 Paimon，模拟 prepareCommit/commit 行为
    // 可注入延迟、失败等异常场景
}
```
...
| WAL 崩溃恢复 - SINK_PREPARE 后 | Kill → 重启 → 使用 CommitMessage 重试 commit |
| WAL 崩溃恢复 - preSink 丢失 | Kill → 重启 → 从本地 SST 重新生成 preSink |
